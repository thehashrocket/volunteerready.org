...
describe('geocodeLocation — with API key', () => {
  ...
  it('returns SUCCESS with lat/lng on a valid response', async () => {
    ...
    expect(result).toEqual({ geocodeStatus: 'SUCCESS', lat: 37.3382, lng: -121.8863 });
  });
...
describe('cleanLocation — suite/floor stripping (via geocodeLocation URL)', () => {
  ...
  it('strips "suite NNN" token', async () => {
    const q = await captureEncodedQuery('100 Market St, Suite 400, San Francisco, CA');
  ...
  it('strips "ste NNN" abbreviation', async () => {
    mockFetch.mockReset();
    mockFetch.mockResolvedValue(okResponse(0, 0));
    const q = await captureEncodedQuery('100 Market St, Ste 400, San Francisco, CA');
  ...
  it('strips "floor N" token', async () => {
    mockFetch.mockReset();
    mockFetch.mockResolvedValue(okResponse(0, 0));
    const q = await captureEncodedQuery('1 Infinite Loop, Floor 3, Cupertino, CA');
