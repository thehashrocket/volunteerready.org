...
export default async function VolunteerPublicProfilePage({
	...
	return (
		<main className="min-h-screen bg-[#FAFAF8]">
			{/* Hero */}
			<section className="border-b border-[#E8E6E1] bg-white px-4 py-12 sm:px-6 lg:px-8">
				<div className="mx-auto max-w-3xl">
					{/* Tenure badge */}
					{profile.tenureLevel !== 'NONE' && (
						<div className="mb-4">
							<Badge className="bg-[#E8DCC8] text-[#3D3B38] hover:bg-[#E8DCC8]">
					...
					<h1 className="font-fraunces text-4xl font-bold text-[#252422] [text-wrap:balance] sm:text-5xl">
						{profile.displayName}
					</h1>

					{/* Location + member since */}
					<div className="mt-3 flex flex-wrap items-center gap-4 text-sm text-[#787571]">
						{location && (
					...
					{/* Bio */}
					{profile.bio && (
						<p className="mt-4 max-w-2xl text-base leading-relaxed text-[#5C5955]">
							{profile.bio}
						</p>
					)}

					{/* Stats row */}
					<div className="mt-8 flex flex-wrap gap-6 border-t border-[#E8E6E1] pt-6">
						<Stat
					...
					{profile.credentials.length === 0 ? (
						<Card>
							<CardContent className="py-10 text-center">
								<ShieldCheck className="mx-auto mb-3 h-8 w-8 text-[#D1CFC8]" />
								<p className="text-sm italic text-[#A8A5A0]">
						...
						<>
							<h2 className="mb-4 font-fraunces text-xl font-semibold text-[#252422]">
			...
			{/* Footer CTA */}
			<section className="border-t border-[#E8E6E1] px-4 py-10 sm:px-6 lg:px-8">
				<div className="mx-auto max-w-3xl">
					<p className="text-sm text-[#A8A5A0]">
						This profile is verified by{' '}
						<Link href="/" className="text-[#1B3C2A] hover:underline">
							VolunteerReady
						</Link>
						.
					</p>
...
function Stat({
	...
	return (
		<div>
			...
			<p className="mt-0.5 text-xs uppercase tracking-widest text-[#A8A5A0]">
				{label}
			</p>
		</div>
	);
...
function CredentialCard({
	...
	return (
		<div className="flex items-start gap-3 rounded-lg border border-[#E8E6E1] bg-white p-4">
			<div className="mt-0.5 shrink-0">
				<ShieldCheck className="h-4 w-4 text-[#1B3C2A]" />
			</div>
			<div className="min-w-0">
				<p className="text-sm font-medium text-[#252422]">{credential.label}</p>
				<div className="mt-1 flex flex-wrap items-center gap-2">
					<span className="flex items-center gap-1 text-xs text-[#787571]">
						<Building2 className="h-3 w-3" />
						{credential.orgName}
					</span>
					{issuedYear && (
						<span className="flex items-center gap-1 text-xs text-[#A8A5A0]">
			...
			<Badge className="ml-auto shrink-0 bg-[#1B3C2A] text-xs text-white hover:bg-[#1B3C2A]">
				Verified
			</Badge>
		</div>
	);
