...
- [x] **T28 (P2, human: ~5h / CC: ~30min)** — ui — Mobile: card list below `lg`, Drawer add form ✅ **DONE**
  ...
  - **The LIST switch is pure CSS (`hidden lg:block` / `lg:hidden`), NOT `useMediaQuery`.** That hook (`lib/hooks/use-media-query.ts:4`) initialises to `false` and only reads `matchMedia` in an effect, so gating layout on it paints the CARD shape to every desktop user and swaps after hydration — the same flash the roster flag has no client read path in order to avoid. The hook is used for the add form and only there: nothing renders until the trigger is pressed, by which point the effect has run. Both trees are always in the DOM; `display: none` also removes one from the accessibility tree, so exactly one `Remove` per volunteer is ever reachable
...
- [x] **T35 (P2, human: ~4h / CC: ~25min)** — ui — `QueryErrorCard` migration + shifts error branch **(TD2, pulled in)** ✅ **DONE** (all four pages, shipped with T28)
  ...
  - **Note the migration is scoped to these four pages only.** ~17 other files under `src/app` still render `{error.message}` raw (`my-applications`, `settings/background-checks`, `admin/platform/*`, `profile`, `apply/status`, …). Out of T35's stated scope, and currently untracked — the "grep for what you are migrating TO" lesson applies to them too
