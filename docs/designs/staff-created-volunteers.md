...
- [x] **T25 (P1, human: ~3h / CC: ~20min)** — ui — Add-volunteer dialog: repeat entry, three success strings, pre-submit hint ✅ **DONE**
  ...
  - The three success strings and the pre-submit hint shipped early, with T11 — this task was only ever the repeat-entry half. Shipped now: the form stays open, all three fields clear, focus returns to Name, a `<output aria-live="polite">` running count appears above the footer, and the footer flips `Cancel`/`Add volunteer` → `Done`/`Add another` on the first success. 17 new unit tests (12 in `AddVolunteerDialog.test.tsx`, 16 → 28; 5 in `page.test.tsx`, 32 → 37) + the e2e batch flow
  - **The live region is mounted EMPTY with the dialog**, with only its content conditional — a region inserted in the same commit as its first text is unreliably announced, so the first add would be the one most likely to go unheard. Not `hidden`/`empty:hidden` while empty either; `display: none` takes it back out of the accessibility tree
  - **`Done` is disabled while a mutation is in flight.** Closing unmounts the form and with it `onSuccess`, so a volunteer the server created gets no toast, no count and no `onAdded()` — the roster behind the form never invalidates and silently omits them. Reachable via Cancel before this ship; `Add another` → `Done` in one motion made it the likely path. Escape and the overlay still orphan the callback — P3, deliberately not closed by trapping the modal
  - **The service needed no change**, despite being listed above. `addVolunteer` was already one call per volunteer; nothing about repeat entry is a server concern
  - ⚠️ **The empty state could not keep its own `AddVolunteerDialog`.** It sits inside the roster page's `volunteers.length === 0` branch, and the first successful add flips that branch — which, once the form stops closing on success, unmounts the form mid-batch and takes the half-typed second volunteer with it. Invisible before this task because the dialog closed on success anyway, and invisible to the unit tests, which render the component directly rather than through the page. `open` therefore moved up to `page.tsx`, there is now exactly ONE dialog, and the empty state renders a shared `AddVolunteerButton` that opens it. **Any future surface that wants an add form must open the page's instance, not mount a second one** — two mounted dialogs would also mean two independent running counts
  - **The toast survives, deliberately.** A footer count could have replaced it, but the toast is the only surface carrying the `We let them know by email.` disclosure, and the byte-identical rule is about the two silent branches being indistinguishable — a toast that fired only for the `LINKED_ACTIVE` branch would break that by its *absence*. `SECURITY: repeat entry does not make the silent branches distinguishable` extends the guarantee to the **running count**. Be precise about what it proves: the client only ever receives `notified`, so both adds in that test are driven with identical arguments and it asserts the count cannot diverge — it is not independent evidence of the two-branch collapse. That evidence is `SECURITY: shadow-mint and other-org-UNCLAIMED produce IDENTICAL copy` (pre-existing) plus the service and router tests, which is what T25's `Verify:` line points at
  - **The batch result survives the form.** Everything confirming a batch landed died at close — the toasts expire (a twenty-name batch runs minutes) and the count unmounts with the dialog — while `refresh()` deliberately keeps `search`, so `Done` could drop the coordinator onto the filtered-empty state, which renders no add button. The page now publishes `N volunteers added to your roster.` above the list on close, through the same `liveMessage` announcer the remove/undo flow uses, with a `Clear search to see them` action when a filter is what is hiding the new rows
  - **The shell is frozen while the form is open.** `Dialog` and `Drawer` are different roots, so crossing `lg` mid-batch unmounted the form and — with `open` lifted to the page — reopened it blank, count reset. An iPad rotating portrait→landscape crosses it. See `docs/TODOS.md` for why this is held by a comment rather than a test
  - The count renders **above** the footer rather than inside it: `DialogFooter` is a right-justified row at this breakpoint and `DrawerFooter` a reversed column, so any single document order puts the count above the buttons in one shell and below them in the other
...
- [x] **T28 (P2, human: ~5h / CC: ~30min)** — ui — Mobile: card list below `lg`, Drawer add form ✅ **DONE**
  ...
  - **Testing note for T36.** Once both trees render, `displayName`, `email` and the badge label each match TWICE — every unscoped `getByText` in `page.test.tsx` had to be rescoped through `within(getByTestId('roster-table'))`. jsdom has no layout, so a CSS breakpoint switch cannot be tested there at all; the unit tests assert both trees exist with the right classes and content, and the 375px e2e is the only thing that proves which one a human sees. Also: `AddVolunteerDialog.test.tsx` needs vaul's pointer-capture polyfills. ~~AND a `getComputedStyle` shim~~ — **corrected in place (v0.41.8.0): the shim is gone.** vaul reads `style.transform || style.webkitTransform || style.mozTransform` and calls `.match()` on it; jsdom 25 yielded `undefined` there, which threw asynchronously and **failed the run while every test still reported green**. jsdom 30 computes the spec-correct `'none'`, so the chain is satisfied natively and the shim was deleted rather than repaired — it also could not have survived, since jsdom 30 brand-checks `CSSStyleDeclaration` methods and the shim's `Object.create(style, …)` wrapper made `getPropertyValue` throw on every `getByRole`
...
- [x] **T36 (P2, human: ~1.5d / CC: ~1h15min)** — ui — Mobile card lists for the four existing staff tables **(TD5)** ✅ **DONE** (2026-07-31)
  - ⚠️ **The pattern did not transfer whole, and the deviation is the important part.** T28's shape is "the row is the whole tap target, actions move into the detail view". Three of these four have no detail view holding the actions: `Publish`/`Close` live on the opportunities LIST and nowhere else (the detail page has only `Edit`), `Complete`/`Cancel`/`Delete` live on the shifts list and nowhere else, and the team page has no detail view at all. Copying the roster literally would have shipped a phone that cannot publish an opportunity or close out a shift. So those three keep their actions on the card and the card is a `<div>` with a linked title, not a `<button>` — which is also the only valid markup, since a nested `<button>` in a tappable row is not. **Applications is the one clean fit** and is a whole-row `<Link>`
  - **Shifts does NOT use `CardList`** — it already sits inside the page's "Shift Schedule" `Card`, so a second one is double chrome. The `-mx-6` workaround tried first failed the e2e as internal sideways scroll, the exact failure these lists remove
  - `CARD_LIST` was extracted to `src/components/app/card-list.tsx` as a `CardList` component at its fifth copy, and `volunteers/page.tsx` migrated onto it
  - Two pre-existing defects fixed in passing, both the same class the roster's `pendingRemovalId` exists for: `OpportunitiesClient` disabled EVERY Publish/Close on any one click, and `settings/team` disabled every row's Select and Remove on any one mutation. Both are now keyed off `mutation.variables`
  - Verify: ✅ **37 new unit tests** across the four pages' existing test files (6 applications / 9 opportunities / 11 team / 11 shifts), scoped through `within(getByTestId(…))` since both trees are always mounted and every string matches twice; plus three files that had NO tests before — `page-header.test.tsx`, `card-list.test.tsx`, `OrgSwitcher.test.tsx`. And `e2e/staff-tables-mobile.spec.ts` — **7 tests**: the four routes at 375px, a desktop-width contrast sweep (all five tables, incl. the roster), a top-bar widest-state sweep across 375/800/1280, and a 768-1023px band sweep. jsdom has no layout engine and evaluates no media query, so the e2e is the only thing that proves which tree a human sees
  - **Depended on the app-shell overflow fix landing first**, not just for tidiness: T36's verify step is "assert no horizontal scroll", and that assertion fails on the shared top bar on every one of these pages. Fixing the shell is what let the assertions be `documentElement`-level rather than region-scoped
...
- [x] **T37 (P2, human: ~1d / CC: ~45min)** — ui — Finish the `safeErrorMessage` migration across the remaining `src/app` surfaces ✅ **DONE (2026-08-01)** — and it was not a UI task
  - **Scope correction:** the premise ("finish the client migration") was wrong. There was no `errorFormatter`, so every message had already crossed the wire before any component chose what to render — `safeErrorMessage` decides what is *painted*, never what is *sent*. The fix is a server-side redaction in `server/trpc/init.ts` off a shared allowlist (`server/domain/error-disclosure.ts`), with the client sweep as the redundant second half. Full write-up in `docs/TODOS.md` under T37
  - **The cost was not in the ~75 client sites; it was in the eighteen user-facing refusals thrown as plain `Error`s**, which tRPC maps to `INTERNAL_SERVER_ERROR` and the formatter therefore redacts. Anything a user is meant to read needs an allowlisted code, and no message-based test can detect the regression
