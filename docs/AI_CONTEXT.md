...
│   ├── trpc/
│   │   ├── init.ts               # Context creation, procedure definitions, and the
│   │   │                           errorFormatter — redacts any message that is not
│   │   │                           BOTH allowlisted and authored by us, and strips
│   │   │                           data.stack on both paths
│   │   ├── error-reporting.ts    # reportTrpcError — the `onError` hook in
│   │   │                           app/api/trpc/[trpc]/route.ts. Runs BEFORE the
│   │   │                           formatter with the raw error, so redaction does not
│   │   │                           also blind us. Skips deliberate refusals and Zod
│   │   │                           input failures; Sentry capture is budgeted per
│   │   │                           procedure+code per minute (which is why
│   │   │                           sentry.server.config.ts sets no global sampleRate)
...
│       ├── error-disclosure.ts    # CLIENT_SAFE_ERROR_CODES, isClientSafeErrorCode(), GENERIC_ERROR_MESSAGE — the one allowlist, read by the errorFormatter, error-reporting.ts and query-error-card.tsx
│       ├── screener/
...
| `src/server/trpc/init.ts` | tRPC context, auth middleware, procedure definitions, and the `errorFormatter` — a message ships only if its code is allowlisted AND we authored it (`error.cause` is not an `Error`); `data.stack` is stripped either way. Not exempted in development, because `pnpm e2e` boots `pnpm dev` |
| `src/server/domain/error-disclosure.ts` | The ONE allowlist of client-safe tRPC error codes — `CLIENT_SAFE_ERROR_CODES`, `isClientSafeErrorCode()` (fails closed), `GENERIC_ERROR_MESSAGE`. Read before adding a `TRPCError` code or an error UI; three consumers share it so the server and client halves cannot drift |
| `src/server/trpc/error-reporting.ts` | `reportTrpcError` — the `onError` hook, running before the formatter with the raw error. Skips deliberate refusals and Zod input failures (public procedures + batching would let curl exhaust the Sentry quota), always `console.error`s, and budgets the Sentry capture per procedure+code per minute |
| `src/components/app/query-error-card.tsx` | `QueryErrorCard`, `safeErrorMessage()` and `safeCaughtErrorMessage()` — the display half. `safeCaughtErrorMessage` exists for `mutateAsync` try/catch, where `err instanceof Error ? err.message : …` looks careful and returns the raw message, because `TRPCClientError extends Error` |
...
13. **Throwing a user-facing refusal as a plain `Error`** — `throw new Error('Cannot remove yourself.')` maps to `INTERNAL_SERVER_ERROR`, which is not on the disclosure allowlist, so since v0.39.0.0 the person reads "Something went wrong. Please try again." instead. A message someone is meant to act on must be a `TRPCError` carrying one of the eight codes in `CLIENT_SAFE_ERROR_CODES` — read them off `src/server/domain/error-disclosure.ts`, which is authoritative, not off a list in a doc. No message-based test catches this — `rejects.toThrow('Cannot remove yourself.')` passes identically for a plain `Error` — so assert the **code**. Never pass both `message:` and `cause:` to one `TRPCError`: the formatter reads a non-null `Error` cause as "not authored by us" and degrades the copy silently. See `src/server/domain/error-disclosure.ts`.
14. **Rendering `error.message` (or `err.message`, or `await res.text()`) into JSX** — a tRPC error can carry internal detail including database text. Use `safeErrorMessage()` / `safeCaughtErrorMessage()` / `QueryErrorCard` from `src/components/app/query-error-card.tsx`; `error-disclosure.guard.test.ts` fails the build on a raw render under `src/app` or `src/components`. Two related traps: `err instanceof Error ? err.message : fallback` returns the raw message, because `TRPCClientError extends Error`; and a *missing* `isError` branch does not fail visibly — it falls through to the empty state, answering a failed load with "No shifts found". Order branches loading → error → empty, and check whether sibling cards share one query before adding a per-card branch.
15. **Gating page LAYOUT on `useMediaQuery`** — it initialises to `false` and only reads `matchMedia` in an effect, so every desktop user gets the mobile tree painted first and swapped after hydration. Table↔card switches are pure CSS (`hidden lg:block` / `lg:hidden`, both trees rendered from the same array). The hook is safe only inside a modal, which mounts after the effect has run — and there it must be frozen while open via `useFrozenDesktopShell`, never re-read live. `src/app/(app)/app/admin/feedback/page.tsx` still violates this.
