...
- `csv.ts` — the repo's ONE CSV parser and writer: `parseCsvRecords` (RFC 4180 — quoted fields, embedded commas and newlines, `""` escapes, BOM, CRLF/LF/CR), `escapeCsvField` + the formula-injection defense, `toCsvLine`, and `unescapeCsvField` (the inverse, because our own export is an input to our own importer). Consumers: ESG export, roster export, roster import, and — since v0.41.0.0 — the applications bulk import, which was the last surface still doing `line.split(',')`. There are now no hand-rolled parsers left; a second one is a regression
...
- `roster-import.ts` — concierge import: column-header aliases, per-row validation reusing `org-volunteer.ts`'s name/email/phone schemas, and (v0.41.0.0) the `--notify-only` correlation — `computeOwedNotices()` joins the CSV's addresses against committed `CONCIERGE_IMPORT` audit rows and `notifyOnlyExitCode()` scores the result. Pure, so which notices are owed is decided without a database
...
- `rosterImportService.ts` — concierge CSV import: per-row transactions through `addVolunteer()` (never a parallel insert), so the block refusal, shadow-user branch and audit row are the same code the add form runs. Writes no audit rows of its own. Opts out of `addVolunteer`'s fire-and-forget notification and paces the sends itself, sequentially and awaited, reading each send's boolean rather than relying on a rejection. Also holds the `--notify-only` recovery path (v0.41.0.0): `classifyOwedNotices()` reconstructs the notices an interrupted run never sent from the org's `CONCIERGE_IMPORT` audit rows, and `sendOwedNotices()` sends them attributed to the actor each audit row recorded. Both notify paths share one `sendNoticesSequentially`, so pacing and failure handling cannot drift between the live run and the recovery
...
- `email.ts` — `sendEmail()` helper — the entry point for outbound email, with **one deliberate bypass**: `sendFcraEmails.ts` calls the Resend SDK directly because those are legally mandated adverse-action notices (recorded in `docs/designs/staff-created-volunteers.md` §3 so it stays a decision, not an oversight). It **returns a boolean and never throws**, so a caller's `try/catch` cannot see a failure; read the return value. Resend does not throw either — its response is `{ data, error }`, so a 429, a bad key and a network failure all resolve normally. v0.41.0.0 added the `result.error` check, before the `EmailEvent` write, so a send that did not happen leaves no SENT row claiming it did
