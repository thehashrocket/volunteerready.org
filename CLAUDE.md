...
- Volunteer self-service leave (T32, v0.36.0.0): `leaveOrgRoster()` in `staffVolunteerService.ts` → `softDeleteOwnOrgVolunteer()` in `orgVolunteerRepo.ts`, surfaced as the "Organizations you volunteer with" card on `/app/profile`. The `userId` sits inside the Prisma `WHERE` of **both** the read and the `updateMany` — mutation-verified: dropping it from one statement leaves the security test green (the other still guards), dropping it from both turns it red, so neither is dead code. Returns the row's `orgId` so the `VOLUNTEER_LEFT` audit row is scoped to the org that loses the volunteer — the client never names an org. Leaving is **narrower than a revocation**: it soft-deletes the `OrgVolunteer` edge only, so a `VolunteerApplication` (and any `ShiftSignup`) still satisfies `requireOrgVolunteerRelationship`, and the org keeps `getOrgVisibleProfile` / `credentials.issue` / `backgroundChecks.initiate` access. The confirm copy says so rather than implying a door that stays shut.
