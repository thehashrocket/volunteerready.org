...
- Claimable-application decline (v0.34.0.0): `declinedByUserId`/`declinedAt` on `VolunteerApplication`, written only by `declineApplicationForUser()`, which enforces the email match **inside the `where`** exactly as the claim does. Filtered out by `listClaimableApplicationsByEmail`, so declining is terminal. Two nullable columns rather than a join table because `User.email` is unique and canonicalized and claimability is email equality — for any orphan row exactly ONE user can ever claim it. The "Not mine" control keeps the same size and variant as "Add to my account" (a test compares their `className`): declining is the safe action here, so it must not read as weaker or more dangerous than accepting.
- Unique-violation narrowing: `isUniqueViolationOn(err, constraint, modelName?)` in `src/server/lib/prisma-errors.ts`. Prisma 7 + the PrismaPg adapter does **not** populate `meta.target`, so the textbook check silently never matches; read `meta.modelName` or `meta.driverAdapterError.cause.originalMessage`. Always narrow to the constraint you mean — mapping any P2002 to one constraint's message tells the caller something false.
