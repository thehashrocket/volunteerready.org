...
- Marketing screenshots: asset manifest at `src/lib/marketing-screenshots.ts` (single source of truth — pages import entries, never hardcode `/marketing/*.png`); annotated imagery via `src/components/annotated-screenshot.tsx` (numbered % -positioned markers + HTML legend, composed by homepage pillar rows and by `ScreenshotSection`'s optional `annotations` prop); capture pipeline at `e2e/capture.spec.ts` + `e2e/capture-scenarios.ts` (typed scenarios: actor/path/clickTabs/waitForText, 1280×720 light-mode, sessions minted via `e2e/utils/db.ts` local-DB guard); CI guards at `src/lib/marketing-screenshots.test.ts` (asset existence + scenario integrity) and scrolled naturalWidth e2e assertions in `e2e/public-pages.spec.ts` (issue #139)
