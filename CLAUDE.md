...
- `OrgSlugHistory` model: past org slugs recorded on rename so old public links (`/apply`, `/opportunities`, `/stories`, OG images) 307-redirect to the current slug via `findCurrentSlugByHistory()` in `orgRepo.ts`; history rows also block slug re-registration (anti-squat)
